...
# กำหนดตัวแปร Fuzzy
temperature = ctrl.Antecedent(np.arange(15, 36, 1), 'temperature')  # ตัวแปร fuzzy สำหรับอุณหภูมิ (°C)
...
temperature['low'] = fuzz.trimf(temperature.universe, [15, 15, 26])    # ฟังก์ชันสมาชิก "low" ของอุณหภูมิ
temperature['high'] = fuzz.trimf(temperature.universe, [24, 35, 35])   # ฟังก์ชันสมาชิก "high" ของอุณหภูมิ
...
fan['low'] = fuzz.trimf(fan.universe, [0, 0, 51])                # ฟังก์ชันสมาชิก "low" ของความเร็วพัดลม
fan['high'] = fuzz.trimf(fan.universe, [49, 100, 100])            # ฟังก์ชันสมาชิก "high" ของความเร็วพัดลม

misting['low'] = fuzz.trimf(misting.universe, [0, 0, 51])        # ฟังก์ชันสมาชิก "low" ของระบบพ่นหมอก
misting['high'] = fuzz.trimf(misting.universe, [49, 100, 100])    # ฟังก์ชันสมาชิก "high" ของระบบพ่นหมอก

led['low'] = fuzz.trimf(led.universe, [0, 0, 51])                # ฟังก์ชันสมาชิก "low" ของไฟ LED
led['high'] = fuzz.trimf(led.universe, [49, 100, 100])            # ฟังก์ชันสมาชิก "high" ของไฟ LED
...
simulation.input['temperature'] = 25  # ตั้งค่าอุณหภูมิ (°C) เพื่อทดสอบ
simulation.input['humidity'] = 60     # ตั้งค่าความชื้น (%) เพื่อทดสอบ
simulation.input['light'] = 50        # ตั้งค่าแสง (%) เพื่อทดสอบ
